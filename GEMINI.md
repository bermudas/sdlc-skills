...
| `servicenow-atf-automation` | ServiceNow ATF full lifecycle via REST — build / run / inspect tests from JSON specs. Installs a one-time Path B Scripted REST API to bypass the `glide_var` filter on `sys_atf_step.inputs` |
