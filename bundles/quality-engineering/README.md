...
> **Not** the `manual-qa` team. `manual-qa` is a standalone run-execution machine —
> onboard the app, author/size cases, run them live, report. `quality-engineering`
> wraps that kind of machine with the in-sprint discipline manual-qa deliberately
> doesn't have: requirement triage, **reuse-first case curation** (update/rewrite
> instead of re-authoring), regression scoping, bugfix verification, and
> requirement↔case↔result↔functionality↔intent triangulation. Use `manual-qa` to
...
| `test-runner` | sonnet | EXECUTE — one dispatch per **session group** (state-compatible cases share one live session with **verified inheritance**; mismatch → full-reset fallback), **bug-hunts** per `ui-bug-hunting` (reads every screenshot, six-pass), emits build-stamped verdicts, and **codifies a clean pass** → `tests/<TC>.spec.ts` + visual baselines. **REPLAY** — cheap deterministic re-run of a codified case (`npx playwright test` + visual-check, near-zero tokens). VERIFY-FIX — original repro re-run on the new build with fresh evidence + regression neighbors. |
...
- **Agents** — the 6 bespoke seats above (bundle-local, manual-qa-style: compact
  bodies, scoped tools, per-seat models) + the shared `scout` (its own copy).
- **Canonical skills authored here** — `case-curation` (the curator's reuse-first
  methodology: classification, contradiction classes, authoring delta, regression
  scoping), **`ui-bug-hunting`** (the runner's *find-bugs-not-greens* discipline —
  Job A/Job B + the six-pass inspection of every screenshot it reads), and
  **`visual-regression-testing`** (agent-as-runner pixel + accessibility-tree
  baselines over CDP, with a replayable scenario for cheap re-runs).
- **Skills** — `requirement-traceability` (triage + 5-axis triangulation),
  ...
  and the **codification** path), `browser-verify`, `reproducing-issues`,
  `verification-before-completion` + `systematic-debugging` (runner/lead
  execution discipline), `issue-tracking` (+ `atlassian-content` for
  Jira/Confluence — the lead writes with it, the story-analyst reads Confluence
  specs + searches similar tickets with it), `xlsx-reader`, scout's seeding set,
  `memory`.
- **Cheap re-execution** — the first EXECUTE of a case is the expensive, bug-hunting
  leg; on a clean pass the runner **codifies** it to an isolated Playwright spec
  (`tests/<TC>.spec.ts`) + visual baselines. Untouched codified cases then re-run
  via **MODE REPLAY** (`npx playwright test` + visual-check) at near-zero tokens —
  the regression scope and bugfix-verify neighbors ride this path; only touched or
  drifted cases pay for a full agent run. The manual case stays the source of
  truth; the spec is a derived, disposable accelerator.
- **Seeded knowledge** — [`knowledge/`](knowledge/) → `.agents/quality-engineering/knowledge/`:
  ...
  adapted from the manual-qa mobile-testing skill, PR #40), and the
