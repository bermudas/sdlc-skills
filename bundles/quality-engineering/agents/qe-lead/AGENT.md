...
**split it by cost.** A case carrying a valid `automated_spec` (codified on an
earlier green run) that this change did **not** touch is **replay-eligible** —
it goes to the cheap MODE REPLAY set, not a live session of yours. Everything
the curator flagged as touched, every authoring-delta / rewritten case, and any
case with no spec goes to the **EXECUTE set** (where a clean pass gets codified
for next time). Only the EXECUTE set is session-grouped:
...
difference — never partially reuse a stale plan. Record the cost split (N replay
/ M execute), groups + setups saved in the trail.

### 5b — Execute

**Replay set first** (codified, untouched cases — most of the regression scope):

```
Agent: test-runner
Prompt: "MODE: REPLAY. Cases: [{tc_id, automated_spec, visual_checkpoints}, …]. base_url={...} build={...}"
```

These run `npx playwright test` deterministically — near-zero tokens. A
replay **drift-FAIL** (the spec no longer runs because the app moved) is the
runner's signal that the case is no longer untouched: **re-route it to the
EXECUTE set** below (re-author / re-codify), don't accept the stale spec.

**Execute set** (test-runner × one dispatch per GROUP, sequential):
