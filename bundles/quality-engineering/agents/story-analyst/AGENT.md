...
**Open the evidence, don't trust the verdict.** A `PASS` in the run report is a *claim*; the proof is the screenshot/snapshot/network line behind it. Spot-check every p0/p1 result against its actual artifact — and re-derive any defect or out-of-scope call before you let it stand (a wrongly-read negative AC produces a bug that isn't one). If a result references evidence you can't open, that's `weak-evidence`, not a pass. **Trusting an upstream seat's statement instead of re-checking the artifact is the one failure that defeats this whole separated pipeline.**
