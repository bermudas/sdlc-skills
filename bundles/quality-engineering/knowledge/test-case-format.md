...
| `automated_spec` | No | **Set by the runner, not the author.** Path to the codified Playwright spec (`tests/TC-001.spec.ts`) emitted on this case's first clean PASS. Its presence makes the case **replay-eligible** — re-runs go through `npx playwright test` (near-zero tokens) instead of a full agent execution. Absent → the case has never passed, or codification was skipped. |
| `codified_build` | No | The build the `automated_spec` was generated against — a staleness signal. The lead won't trust a spec older than a change that touches the case. |
| `visual_scenario` | No | Path to the `scenarios/TC-001.json` + visual baselines (`visual-regression-testing`) when the case has a visual surface — replayed for visual-regression drift on re-runs. |
...
## Automation accelerators — codified replay (set by the runner)

The **manual case stays the source of truth.** But a case that has passed once
shouldn't cost a full agent execution every regression. On a case's first clean
PASS the `test-runner` **codifies** it (see the runner's "codify" step): it emits
a Playwright spec (`tests/{id}.spec.ts`, the steps as actions + each Expected
Result as an assertion) and stamps `automated_spec` + `codified_build` into the
case's frontmatter; for a case with a visual surface it also captures
`visual-regression-testing` baselines + a `visual_scenario`. Thereafter the lead
routes an **untouched** codified case to a cheap MODE REPLAY (`npx playwright
test` + visual-check) instead of re-driving it. The spec is a **derived,
disposable accelerator** — when the change touches the case, or the spec drifts
and won't run, the runner falls back to a full execution and re-codifies. Never
hand-edit `automated_spec`/`codified_build`; they are the runner's bookkeeping.
