...
## Step 6.97 — @-import wiring verification

Every installed non-scout agent declares a load hook at the top of its
body (immediately after the YAML frontmatter):

```markdown
@.agents/memory/<role>/MEMORY.md
@.agents/profile.md
@.agents/workflow.md
@.agents/testing.md         # qa, TAE, TAL only
@.agents/architecture.md    # tech-lead, devs
@.agents/conventions.md     # devs only
@.agents/team-comms.md
```

These `@`-imports are how scout's outputs reach the agent at session
start. Without them, the agent runs blind to project context (the failure
mode 2026-05-13 PM session demonstrated: defaults overrode scout findings
because scout findings were never auto-loaded).

After Step 6.95 (deployment-mode strip), verify each installed agent's
load-hook block matches the role's required imports. Required imports
per role:

| Role | Required imports |
|---|---|
| `project-manager` | `MEMORY.md`, `profile.md`, `workflow.md`, `team-comms.md` |
| `test-automation-lead` | `MEMORY.md`, `profile.md`, `workflow.md`, `testing.md`, `team-comms.md` |
| `tech-lead` | `MEMORY.md`, `profile.md`, `workflow.md`, `architecture.md`, `team-comms.md` |
| `qa-engineer` | `MEMORY.md`, `profile.md`, `workflow.md`, `testing.md`, `team-comms.md` |
| `test-automation-engineer` | `MEMORY.md`, `profile.md`, `workflow.md`, `testing.md`, `team-comms.md` |
| `ba` | `MEMORY.md`, `profile.md`, `workflow.md`, `team-comms.md` |
| `js-dev` / `python-dev` / `ios-dev` | `MEMORY.md`, `profile.md`, `workflow.md`, `conventions.md`, `team-comms.md` |
| `scout` | `MEMORY.md` only (scout produces the rest — chicken/egg) |
| `personal-assistant` | unchanged (not a project agent) |

### What to verify

1. **The `@`-import block exists** between the closing `---` of YAML frontmatter and the first `#` heading.
2. **`MEMORY.md` exists at the imported path** (you've seeded it in Step 7c — see [`references/templates.md`](references/templates.md) § Index file).
3. **The project-context files referenced exist** OR scout skipped them deliberately (e.g., no `conventions.md` for a project where no patterns were detected). Missing imports resolve to non-fatal warnings — that's tolerated. What's NOT tolerated: the import line being absent from the agent file when scout DID produce the corresponding `.agents/<file>.md`.

### Frontmatter `skills:` audit (companion check)

While verifying imports, also audit each agent's `skills:` frontmatter:

1. **Ghost references** — declared skills that don't resolve to a present
   `.claude/skills/` directory and aren't catalogued in
   `<install>/skills.json`. Surface as:
   ```
   ⚠️ Frontmatter ghost references:
     qa-engineer: `foo-skill` (not in .claude/skills/ and not in skills.json)
   ```

2. **Coverage gaps** — skills the project's detected scope requires that
   aren't declared in the relevant role's frontmatter. Map:

   | Project scope marker | Required skills, per role |
   |---|---|
   | `tests/` or `e2e/` directory exists | TAL, qa-engineer, test-automation-engineer: `test-automation-workflow`; qa-engineer + TAE: `test-case-analysis`, `playwright-testing` (or framework-equivalent) |
   | Issue tracker = Jira | PM, TAL, qa-engineer, TAE, tech-lead, scout, ba, devs: `atlassian-content` |
   | TMS = Xray | qa-engineer + TAE: `xray-testing` |
   | Bug-filing convention defined | TAL + qa-engineer: `issue-tracking` (plus tracker-specific authoring skill) |
   | Code review required pre-merge | PM, TAL, tech-lead, qa-engineer: `code-review` |
   | Browser automation | qa-engineer + TAE: `browser-verify`, `playwright-cli` |

   Surface gaps as:
   ```
   ⚠️ Frontmatter coverage gaps:
     project-manager: project scope = "feature-development + test-automation"
       - missing: (none — PM no longer carries test-automation skills; TAL handles)
     test-automation-lead: missing — install via `--agents test-automation-lead`
     qa-engineer:
       - missing: test-automation-workflow
   ```

### Output

Append a section to the onboarding report (and to
`.agents/audit/skills-frontmatter-audit.md` if the project wants an audit
trail) summarizing both checks. Format:

```markdown
## Frontmatter + @-import audit (auto-generated by scout)

**Detected scope:** test-automation, Jira tracker, qaspace TMS, Playwright + TypeScript

**Ghost references:** none / [list]

**@-import wiring:** complete / [list of missing imports per role]

**Frontmatter coverage gaps:**
- project-manager: needs (none — TAL absorbs TA)
- test-automation-lead: not installed (recommend install)
- qa-engineer: needs `test-automation-workflow`, `code-review`

**Recommendation:**
- Install `test-automation-lead`: `npx github:arozumenko/sdlc-skills init --update --agents test-automation-lead`
- Open a `chore/agent-frontmatter-skills-coverage` PR adding missing skills to affected agents
```

Scout surfaces these findings in its terminal output AND offers to write
the PR if the operator approves.

### When NOT to flag

- **Partial project-context files.** If scout produced `.agents/profile.md`
  but not `.agents/conventions.md`, that's fine — the agent's
  `@.agents/conventions.md` import resolves to a non-fatal warning and
  the agent proceeds.
- **Personal-assistant agent.** Not a project agent. Skip.
- **Scout itself.** Only imports its own `MEMORY.md`.
