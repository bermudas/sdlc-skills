...
| [references/poc-bench-patterns.md](references/poc-bench-patterns.md) | **Reusable patterns proven on real ATF benches**: ACL outcome differential via `enforce_security=true/false`, destructive-cascade substitution via `run_server_script` + count-back anchors, server-side REST cleanup for tables without Agent DELETE ACL, the hybrid-coverage decision matrix (what ATF covers cleanly vs what stays with Playwright/Cypress at the UI layer), `simple_name_values` normalization, operational patterns (single-runner state accumulation, pre-flight scope check, whitelist substring discipline, aria-vs-text). **Read this for "what does this skill actually do well, what does it not, and how do I plan a hybrid test from day 1"** — cross-referenced against ServiceNow's own docs and community pattern catalogs. |
